...
func NewLedgerWithRetention(retainedBlocks int) *Ledger {
	return NewLedgerWithRetentionAndMetrics(retainedBlocks, nil)
}

func NewLedgerWithRetentionAndMetrics(retainedBlocks int, metrics *Metrics) *Ledger {
	return &Ledger{
		nextBlockNum:   1,
...
func (l *Ledger) SubscribeEventBatches() ([]*committerpb.TxEventBatch, <-chan *committerpb.TxEventBatch, func()) {
	...
	l.recordEventSubQueueDepthLocked()
	l.mu.Unlock()
...
func (l *Ledger) recordEventSubQueueDepth() {
	l.mu.RLock()
	defer l.mu.RUnlock()
	l.recordEventSubQueueDepthLocked()
}

func (l *Ledger) recordEventSubQueueDepthLocked() {
	depth, capacity := 0, 0
	...
	l.metrics.RecordEventSubQueueDepth(depth, capacity)
}
